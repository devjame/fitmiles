<aside class="relative bg-primary h-screen w-64  sm:block shadow-xl transition duration-150 ease-in-out">
    <!-- Logo -->
	<div class="flex-shrink-0 flex items-center p-6">
		<a href="{{ route('dashboard') }}">
			<x-application-logo class="block h-10 w-auto fill-current text-secondary" />
		</a>
		<span class="pl-4 text-2xl uppercase text-white font-bold hidden sm:inline-block">FitMiles</span>
	</div>
	...
	<x-sidebar-link class="w-full" :href="route('dashboard')" :active="request()->routeIs('dashboard')">
		...
		<span class="hidden sm:inline-block pl-4">{{ __('Dashboard') }}</span>
	</x-sidebar-link>
	...
	<x-sidebar-link class="w-full" :href="route('modalidades')" :active="request()->routeIs('modalidades')">
		...
		<span class="hidden sm:inline-block pl-4">{{ __('Modalidades') }}</span>
	</x-sidebar-link>
</nav>
